...
export const createNotification = async (notificationData) => {
  try {
    ...
    // NEW: Send push notification to all admins in background (fire-and-forget)
    // Don't block notification creation if push fails
    try {
      // Prepare payload for push notification API
      const pushPayload = {
        type: notificationData.type,
        message: notificationData.message,
        orderId: notificationData.orderId,
        orderShortId: notificationData.orderShortId,
        customerName: notificationData.customerName,
        amount: notificationData.amount,
      };

      // Call server-side API to send push (uses firebase-admin on server)
      await fetch('/api/notifications/send-push', {
        ...
        // Log but don't throw - notification was created successfully
        console.error('Push notification API call failed:', err);
      });
    } catch (pushError) {
      console.error('Failed to send push notification (notification still created):', pushError);
