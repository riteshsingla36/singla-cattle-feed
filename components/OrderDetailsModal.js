    ...
    if (window.ReactNativeWebView) {
      ...
      try {
        ...
        window.ReactNativeWebView.postMessage(JSON.stringify({
          ...
          base64: base64Data,
          filename: `Invoice_${freshOrder.id.substring(0, 10).toUpperCase()}.pdf`
        }));
      } catch (error) {
    ...
    try {
      ...
      if (!freshOrder.billPdfUrl) {
        ...
        fetch('/api/upload-bill', { method: 'POST', body: formData })
          ...
          .then(data => {
            if (data.success) {
              saveBillUrl(freshOrder.id, data.url);
              setFreshOrder(prev => ({ ...prev, billPdfUrl: data.url }));
            }
          })
          ...
          {freshOrder.paymentScreenshotUrl && (
            <div className="border-t pt-6 dark:border-gray-700">
              ...
              <div className="flex justify-end mb-2">
                <button
                  onClick={() => handleDownloadPaymentScreenshot(freshOrder.paymentScreenshotUrl)}
                  disabled={loading}
                  className="inline-flex items-center space-x-2 px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
              ...
              <div className="overflow-hidden flex justify-center bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
                <img
                  ...
                  className="max-w-full rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-md max-h-[50vh] object-contain"
                />
              </div>
            </div>
          )}
